...
Bulk of the code has been generated by modules:
 
 - One is responsible generating the skeletons dicts based on the occ api
 - A second one is generating all the class files from those dicts
 - A third one creates the cli interface and help

### _How it works_

Every generated class inherits from a base(NcOcc) that does the heavy lifting.
(As a side effect, it's easier to maintain)
